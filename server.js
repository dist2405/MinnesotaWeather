...
app.get('/:selected_template', (req, res) => {
    fs.readFile(path.join(template_dir, req.params.selected_template + '.html'), 'utf-8', (err, template) => {
        ...
        switch (req.params.selected_template) {
            case 'weather':
                query = 'SELECT name as weather, COUNT(date_time) as num_of_storms, strftime(\'%Y\', date_time) as year, SUM(deaths_direct) as direct_deaths, SUM(injuries_direct) as direct_injuries, SUM(damage_property) as property_damage_$, SUM(damage_crops) as crop_damage_$ FROM Users LEFT JOIN Types ON Users.type = Types.id ';
                if(req.query['group']) {
                ...
                break;
            case 'year':
                query = 'SELECT strftime(\'%Y\', date_time) as year, Types.name as type, COUNT(date_time) as num_of_storms, SUM(deaths_direct) as direct_deaths, SUM(injuries_direct) as direct_injuries, SUM(damage_property) as property_damage_$, SUM(damage_crops) as crop_damage_$ FROM Users LEFT JOIN Types ON Users.type = Types.id ';
                ...
                break;
            case 'county':
                query = 'SELECT replace(cz_name, \' CO.\', \'\') as county, Types.name as type, COUNT(date_time) as num_of_storms, SUM(deaths_direct) as direct_deaths, SUM(injuries_direct) as direct_injuries, SUM(damage_property) as property_damage_$, SUM(damage_crops) as crop_damage_$ FROM Users LEFT JOIN Types ON Users.type = Types.id ';
